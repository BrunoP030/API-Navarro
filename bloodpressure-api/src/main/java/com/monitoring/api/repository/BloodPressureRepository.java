...
public interface BloodPressureRepository extends JpaRepository<BloodPressureData, Long> {
    ...
    @Query("SELECT b FROM BloodPressureData b WHERE b.patientId = :patientId AND b.systolic < :maxSystolic ORDER BY b.id DESC")
    ...
    @Query("SELECT b FROM BloodPressureData b WHERE b.patientId = :patientId AND b.diastolic < :maxDiastolic ORDER BY b.id DESC")
    ...
    @Query("SELECT b FROM BloodPressureData b WHERE b.patientId = :patientId AND (b.systolic < 90 OR b.diastolic < 60) ORDER BY b.id DESC")
    ...
    List<BloodPressureData> findByPatientIdAndDeviceId(String patientId, String deviceId);
}
